...
处理步骤：
1. 纠错：结合语境和你掌握的常识，修正明显的同音字和识别错误。产品名、模型名、人名、品牌名等专有名词按其业界标准写法输出；同一专有名词在文中多次出现时，统一为其中最合理、最标准的写法。当识别结果中出现不像真实词语的音节组合（如把专有名词听成同音字拼凑："僵客""Q千万"），必须按语境和常识推断其应有的词并修正，不要因为"保持原意"而放过明显错误；只有实在无法推断时才保留原样。
...
1. 去重：相邻两段尾部与头部是同一段语音，需合并重叠区，保留上下文更完整、出现位置更靠后的版本；若两段措辞差异较大，保留语义更准确、更通顺的那段，不要生硬拼接。
2. 空段处理：若某段内容为空（行末没有可识别的文字，代表该段是静音或无语音），跳过该段，不要输出空行，也不要保留 [段N] 标记，只输出有内容的段拼接后的结果。
3. 纠错：结合语境和你掌握的常识，修正明显的同音字和识别错误。产品名、模型名、人名、品牌名等专有名词按其业界标准写法输出；同一专有名词在多段中多次出现时，统一为其中最合理、最标准的写法。当识别结果中出现不像真实词语的音节组合（如把专有名词听成同音字拼凑："僵客""Q千万"），必须按语境和常识推断其应有的词并修正，不要因为"保持原意"而放过明显错误；只有实在无法推断时才保留原样。
