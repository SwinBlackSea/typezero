...
处理步骤：
1. 纠错：结合语境和你掌握的常识，修正明显的同音字和识别错误。专有名词（人名、品牌、产品、模型名等）常被语音识别成读音相近的中文词，且该中文词本身可能是真实词语（如"轻微""高开"可能对应某个英文产品名）；必须结合上下文推断它最可能指哪个专有名词，并按业界标准写法输出（含正确大小写，如 Qwen 不是 qwen）。同一专有名词多次出现时统一写法；文中同时出现多个专有名词时，要保持它们之间的区分，不能合并。修正以使整句语义通顺、符合常见表达为准：若某个识别结果使句子明显不通顺（如"这不是环科ASR能解决的"），应按常见搭配还原（"这不是换个 ASR 能解决的"），不要保留不通顺的写法。不要因为"保持原意"而放过明显错误；实在无法推断时才保留原样。
...
输入：高开和千问我都试过，高开速度更快
输出：Groq 和 Qwen 我都试过，Groq 速度更快

输入：嗯那个就是说这个方案是可行的然后成本的话也不会很高
输出：这个方案是可行的，成本也不会很高。

输入：我用 Q千万来识别，速度比僵客快
输出：我用 Qwen 来识别，速度比 Groq 快
...
1. 去重：相邻两段尾部与头部是同一段语音，需合并重叠区，保留上下文更完整、出现位置更靠后的版本；若两段措辞差异较大，保留语义更准确、更通顺的那段，不要生硬拼接。
2. 空段处理：若某段内容为空（行末没有可识别的文字，代表该段是静音或无语音），跳过该段，不要输出空行，也不要保留 [段N] 标记，只输出有内容的段拼接后的结果。
3. 纠错：结合语境和你掌握的常识，修正明显的同音字和识别错误。专有名词（人名、品牌、产品、模型名等）常被语音识别成读音相近的中文词，且该中文词本身可能是真实词语（如"轻微""高开"可能对应某个英文产品名）；必须结合上下文推断它最可能指哪个专有名词，并按业界标准写法输出（含正确大小写，如 Qwen 不是 qwen）。同一专有名词多次出现时统一写法；文中同时出现多个专有名词时，要保持它们之间的区分，不能合并。修正以使整句语义通顺、符合常见表达为准：若某个识别结果使句子明显不通顺（如"这不是环科ASR能解决的"），应按常见搭配还原（"这不是换个 ASR 能解决的"），不要保留不通顺的写法。不要因为"保持原意"而放过明显错误；实在无法推断时才保留原样。
