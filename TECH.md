...
- 日志仅记录请求 ID、总耗时、接收校验/识别/润色三个阶段的耗时、供应商结果和错误码，不记录音频内容、完整文字或 API Key。开发/测试阶段额外输出 `dictation.test_metrics` 会话指标：段数、每段音频时长、每段 ASR 耗时、首段到达至全部段识别完成的总跨度、合并润色耗时（均不含音频内容与文字）；正式发布前可按需关闭该日志。客户端仅在当前界面展示本次请求的构造与 HTTP 往返时间，以及服务端通过 `Server-Timing` 返回的阶段耗时，不持久化这些指标。
...
- 对比模式：`ASR_COMPARE=1` 时服务端把每段音频同时发给主/备两个 ASR，逐段记录各自耗时与原始转写，并在会话结束时用同一 DeepSeek 润色备选结果，全部写入 `ASR_COMPARE_FILE`（默认 `/tmp/asr_compare.jsonl`，仅测试期开启，不进 server.log）。注意 `GROQ_LANGUAGE=zh` 会拖累英文识别（实测英文样本被听成中文乱码）。
- 60s 对比实测（`ASR_COMPARE=1`，同一英文音频拆两段 30s）：Groq 每段 0.7~1.3s、Qwen 每段 63~65s（合计约 128s）；Qwen 两段转写全对，Groq 第一段英文正确、第二段被 `GROQ_LANGUAGE=zh` 带偏成中文乱码（行为不稳定）。结论：速度 Groq 碾压、英文准确率 Qwen 明显更好；中文场景 Groq 依赖热词兜底。产品建议：交互听写保持 Groq，Qwen 作批量精校备选；后续可试 Groq `language` 自动检测解决中英混说。
...
已实现于 `chunked-dictation-ds`。录音期间每凑满一个窗口即在后台上传做 ASR；停止录音时只剩末段 ASR 与合并润色，耗时与录音长度无关（约末段 ASR + 1 秒合并）。录音中不展示识别文字，不违反"首版不做实时字幕"。

### 阶段 3 协议与限流约束（实现要点）

- **分段是必需而非可选**：官方 Qwen-ASR API 单次音频限制 3 分钟 / 10 MB，整段 5 分钟录音直接上传不合规。每段固定 30 秒窗 + 2 秒重叠，远低于限制。
...
- **429/5xx 退避重试**：Qwen provider 对限流和服务端错误做 3 次带退避的重试；网络/超时错误不重试，避免把慢调用叠加成多次超时。
- **坏块不连坐**：某段 ASR 硬失败只返回该段错误，不删除会话；客户端停止后重传失败段，全部成功后末段才触发合并。会话由 TTL（默认 5 分钟）兜底清理。
