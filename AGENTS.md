...
不要用 `NSHostingView` 包裹 SwiftUI View（`onTapGesture` 不响应），不要用 `statusItem.button.action`（macOS 12 不触发）。弹 `NSPopover` 前先 `NSApp.activate(ignoringOtherApps: true)` 并把 `show` 延到下一 RunLoop，否则面板可能不显示或闪退。
